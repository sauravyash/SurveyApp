...
// --- Other/Non-binary algorithm (updated from Excel GP sheet) ---
export function otherGenderAlgorithm(inputs: Inputs): Scores {
  ...
  const alcohol = (inputs.alcohol ?? 0) as Alcohol;
  const smoking = (inputs.smoking ?? 'non-smoker') as Smoking;
  ...
  switch (age_cat) {
    ...
    case '90+':   dementia_score += 49; stroke_score += -6; mi_score += 9; break;
    case 'unknown': default: break;
  }

  // EDUCATION
  switch (edu_cat) {
    case 'less than secondary': dementia_score += 7; stroke_score += -1; diabetes_score += 1; break;
  ...
  // BMI
  switch (bmi) {
    case 'underweight':  dementia_score += 11; stroke_score += -1; mi_score += 12; break;
    case 'normal weight': break;
    case 'overweight':   stroke_score += 2; mi_score += 5; diabetes_score += 12; break;
    case 'obese':        dementia_score += -1; stroke_score += 1; mi_score += 4; diabetes_score += 20; break;
    case 'unknown': default: break;
  }

  // CONDITIONS
  if (isYes(diabetes))     { stroke_score += 3; mi_score += 5; }
  if (isYes(hbp))          { stroke_score += 4; mi_score += 1; diabetes_score += 4; }
  if (cholesterol === 'high') { dementia_score += 4; stroke_score += 6; mi_score += -3; diabetes_score += 6; }
  if (isYes(tbi))          { stroke_score += 2; mi_score += 1; }
  ...
  if (sleepSum > 15) { stroke_score += 5; mi_score += 10; diabetes_score += 3; }
