...
/** GP — OTHER / NON-BINARY (updated from Excel GP sheet) */
export function otherGenderAlgorithm(inputs: Inputs): Scores {
  ...
  const alcohol = inputs.alcohol ?? 0;
  ...
  // CONDITIONS
  if (isYes(diabetes)) { dementia_score += 0; stroke_score += 3; mi_score += 5; }
  ...
  if (isYes(tbi)) { dementia_score += 0; stroke_score += 2; mi_score += 1; }
  ...
  // ALCOHOL
  switch ((alcohol as number) as 0 | 1 | 2) {
